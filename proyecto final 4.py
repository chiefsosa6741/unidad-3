...
def mostrar_ticket(producto, precio, cantidad, total):
  ...
  ticket.geometry("600x700")
  ticket.resizable(True)
...
try:
   BASE_DIR = os.path.dirname(os.path.abspath(__file__))
   imagen = Image.open(os.path.join(BASE_DIR, "haz un logo d un proyecto que tengo de hacer una base de datos que tome aportaciones (1) (1).jpg"))
...
# -------------------------
estilo = ttk.Style()
estilo.configure("TButton", font=("Arial", 12), padding=10,background = '#0B78F4', foreground = '#777A7D')
